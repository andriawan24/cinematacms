...
describe('Switch', () => {
	...
	it('uses cyan track and moved thumb when checked', () => {
		...
		const track = screen.getByRole('checkbox', { name: 'AUTOPLAY' }).parentElement.querySelector('[data-switch-track]');
		const thumb = screen.getByRole('checkbox', { name: 'AUTOPLAY' }).parentElement.querySelector('[data-switch-thumb]');
	...
	it('supports dynamic width while keeping thumb size and padding defaults', () => {
		render(<Switch checked width={52}>AUTOPLAY</Switch>);
	...
	it('fires change when toggled', async () => {
		...
		render(<Switch checked={false} onChange={handleChange}>AUTOPLAY</Switch>);
